...
void triColor(int x = 1, int y = 1) {
	...
	for (int i = 1; i <= k && !doneTriColor; i++) {
		if (x > 1 && a[x - 1][y] == i) continue;
		if (y > 1 && pattern[x][y] == pattern[x][y - 1] && a[x][y - 1] != i) continue;
		if (y > 1 && pattern[x][y] != pattern[x][y - 1] && a[x][y - 1] == i) continue;
		if (limit[i][sizeP[x][y]] == 0) continue;
...
int main() {
	...
	makeTriple({12, 3});
	makeTriple({4, 4, 4, 3});
	makeTriple({2, 2, 2, 2, 2, 2, 3});
	triColor();
	
	QUERY();
	UPDATE();
	
	vector<int> query2 = {1, 1, 1, 2, 2, 2, 3, 3, 3, 1, 1, 2, 2, 3, 3, 1, 2, 3};
	int tmp2 = 0;
