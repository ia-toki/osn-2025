...
int doInteraction() {
	if (scanf("%d", &N) != 1) return WA("Cannot read N");
	if (N < MIN_N || N > MAX_N) return WA("N must be between 6 and 50");
	...
	while (true) {
		char op[101];
		if (scanf("%99s", op) != 1) return WA("Cannot read 'QUERY' or 'ANSWER'");
		
		if (op == ANSWER) {
			int X, Y;
			if (scanf("%d", &X) != 1) return WA("Cannot read X");
			if (X < 1 || X > N) return WA("X must be between 1 and N");
			if (scanf("%d", &Y) != 1) return WA("Cannot read Y");
			if (Y < 1 || Y > N) return WA("Y must be between 1 and N");
			...
			if (checkAnswer(X, Y) == false) {	
				...
				char excess[101];
				if (scanf("%99s", excess) == 1) return WA("The program should terminate after answering");
			...
			numQuery++;
			if (numQuery > QUERY_LIMIT) return WA("Only up to 3 queries are allowed");
			
			for (int i = 1; i <= N; i++) {
				for (int j = 1; j <= N; j++) {
					if (scanf("%d", &A[i][j]) != 1) return WA("Cannot read grid A completely");
				}
			}
			
			if (checkValidQueryInPlay() == false) {
				return WA("NON-STERILIZED cell must contain an integer between 1 and 36");
			
			} else if (checkValidQueryNotInPlay() == false) {
				return WA("STERILIZED cell must contain 0");
			...
			return WA("Cannot read 'QUERY' or 'ANSWER'");
		}
	}
}
